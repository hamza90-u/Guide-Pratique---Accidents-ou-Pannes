...
export function EmergencyContacts() {
  return (
    <Card id="emergency-contacts" className="scroll-mt-24 w-full overflow-hidden">
      <CardHeader>
        <CardTitle className="font-headline text-xl md:text-2xl">Contacts d'urgence</CardTitle>
      ...
      <CardContent className="grid gap-3 sm:gap-4">
        {contacts.map((contact) => (
          <a key={contact.name} href={`tel:${contact.tel}`} className="block">
            <Button variant="outline" className="w-full h-auto justify-start p-3 sm:p-4 text-left">
              <div className="flex items-center gap-3 sm:gap-4 w-full">
                <div className="flex h-10 w-10 items-center justify-center rounded-md bg-destructive/10 text-destructive flex-shrink-0">
